...
func Test_NewSimpleFileServer(t *testing.T) {
	...
	// Test #1.
	result, err = NewSimpleFileServer("non-existent-folder", true, 0, 0, 60)
	aTest.MustBeAnError(err)
	aTest.MustBeEqual(result, (*SimpleFileServer)(nil))

	// Test #2.
	existingDataFolder := filepath.Join("test", "data")
	result, err = NewSimpleFileServer(existingDataFolder, true, 100, 1_000_000, 60)
	aTest.MustBeNoError(err)
	aTest.MustBeDifferent(result, (*SimpleFileServer)(nil))
}

func Test_getFileContentsUsingCache(t *testing.T) {
	var aTest = tester.New(t)
	var err error
	var sfs *SimpleFileServer
	existingDataFolder := filepath.Join("test", "data")
	sfs, err = NewSimpleFileServer(existingDataFolder, true, 100, 1_000_000, 60)
	aTest.MustBeNoError(err)
	var data []byte
	var fileExists bool
...
func Test_getFileContentsWithoutCache(t *testing.T) {
	var aTest = tester.New(t)
	var err error
	var sfs *SimpleFileServer
	existingDataFolder := filepath.Join("test", "data")
	sfs, err = NewSimpleFileServer(existingDataFolder, false, 0, 0, 0)
	aTest.MustBeNoError(err)
	var data []byte
	var fileExists bool
